...
char* decode(const char* code, int* filesize)
{
	...
	*filesize = len / 4 * 3 - flag;
	...
	int i = 0;
	for (; i < loop; ++i) {
		char c0 = table[(int)code[i * 4 + 0]]; assert(c0 >= 0);
		char c1 = table[(int)code[i * 4 + 1]]; assert(c1 >= 0);
		char c2 = table[(int)code[i * 4 + 2]]; assert(c2 >= 0);
		char c3 = table[(int)code[i * 4 + 3]]; assert(c3 >= 0);
